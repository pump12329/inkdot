...
describe('思维导图示例测试', () => {
  ...
  describe('基础节点操作', () => {
    it('应该能够创建和管理节点', () => {
      ...
      // 创建节点
      const createdNodes = testNodes.map(node => 
        engine.createNode(node.content, node.position)
      )
  ...
  describe('渲染性能测试', () => {
    it('应该能够快速渲染大量节点', () => {
      ...
      console.log(`渲染${nodes.length}个节点和${connections.length}个连接耗时: ${duration.toFixed(2)}ms`)
    })
