...
export class TestDataGenerator {
   ...
   * 生成测试节点
   */
  static generateNodes(count: number, options: {
    prefix?: string
   ...
   * 生成AI响应测试数据
   */
  static generateAIResponse(options: {
    success?: boolean
    content?: string
    error?: string
  } = {}): ChatResponse {
  ...
  static generatePerformanceTestData(scale: 'small' | 'medium' | 'large' | 'xlarge') {
    ...
    return {
      ...
      connections: this.generateConnections(
        this.generateNodes(config.nodes), 
        config.connections
      )
    }
  }
   ...
   * 生成随机内容
   */
  private static generateRandomContent(prefix: string, minLength: number, maxLength: number): string {
